...
mod tests {
    ...
    fn test_kdtree_nearest() {
        let items = vec![
            (Point2D::new(0.0, 0.0), "origin"),
            (Point2D::new(1.0, 0.0), "x"),
            (Point2D::new(0.0, 1.0), "y"),
            (Point2D::new(1.0, 1.0), "xy"),
        ];

        let tree = KdTree::from_items(items);

        // Query near origin
        let result = tree.nearest_neighbor(&Point2D::new(0.1, 0.1));
        assert_eq!(result, Some(&"origin"));

        // Query near (1, 1)
        let result = tree.nearest_neighbor(&Point2D::new(0.9, 0.9));
        assert_eq!(result, Some(&"xy"));
    }

    #[test]
    fn test_segment_projection() {
        let seg = Segment::new(Point2D::new(0.0, 0.0), Point2D::new(2.0, 0.0));

        // Point above middle of segment
        let (proj, t) = seg.project_point(&Point2D::new(1.0, 1.0));
        assert!((proj.x - 1.0).abs() < 1e-10);
        assert!((proj.y - 0.0).abs() < 1e-10);
        assert!((t - 0.5).abs() < 1e-10);

        // Point before segment start
        let (proj, t) = seg.project_point(&Point2D::new(-1.0, 0.0));
        assert!((proj.x - 0.0).abs() < 1e-10);
        assert!((t - 0.0).abs() < 1e-10);

        // Point after segment end
        let (proj, t) = seg.project_point(&Point2D::new(3.0, 0.0));
        assert!((proj.x - 2.0).abs() < 1e-10);
    ...
    fn test_segment_index() {
        let segments = vec![
            (
                Segment::new(Point2D::new(0.0, 0.0), Point2D::new(1.0, 0.0)),
                "horizontal",
            ),
            (
                Segment::new(Point2D::new(0.0, 0.0), Point2D::new(0.0, 1.0)),
                "vertical",
            ),
        ];

        let index = SegmentIndex::bulk_load(segments);

        // Query near horizontal segment
        let result = index.nearest_segment(&Point2D::new(0.5, 0.1));
        assert!(result.is_some());
        let (_, data, _, _) = result.unwrap();
        assert_eq!(*data, "horizontal");

        // Query near vertical segment
        let result = index.nearest_segment(&Point2D::new(0.1, 0.5));
        assert!(result.is_some());
        let (_, data, _, _) = result.unwrap();
        assert_eq!(*data, "vertical");
    }
}
